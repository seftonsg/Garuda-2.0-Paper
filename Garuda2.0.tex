...
%%%%%%%%%%%%%%%%%%%%%%%%
  \section{Compilation to Verilog}\label{sec:comp}
    ...
    \subsection{The Intermediate Language}\label{sec:comp:int}
      Refer to Figure~\ref{fig:compile:int} through this explanation for the Extended BNF~\cite{ISO14977} definition of the language.
      We define values and registers as the primitives for the language.
      Values are somewhat similar to the instruction and result fields of \oldname in Figure~\ref{fig:garuda:defn}.
    ...
    \subsection{Compiling \sysname to the Intermediate}\label{sec:comp:comp}
      ...
      \subsubsection{Compiling Predicates}\label{sec:comp:comp:pred}
        We begin by defining the compilation of Falsity in Figure~\ref{fig:compile:compile:pred}.
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%
      \subsubsection{Compiling Policies}\label{sec:comp:comp:pol}
        ...
        Starting immediately with the slices, Figure~\ref{fig:compile:compile:pol} shows that each only needs to declare two new buffers.
