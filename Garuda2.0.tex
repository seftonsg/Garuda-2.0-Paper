...
%%%%%%%%%%%%%%%%%%%%%%
  ...
  %   \sysname was initially designed with the goal of making the definitions of security policies available to more users.
  ...
  %   The solution to the first problem was clear in my mind: create a language specifically for defining security policies for the hardware.
  %   This language would act like a standard scripting language, but be stripped to its essentials.
  %   With the help of Dr. Gordon Stewart, we were able to deduce that a Kleene Algebra with Tests was the best base model.
  %   Many programmers are familiar with some form of regular expression language.
  %   As regular expressions are simply a subset of Kleene algebras, our language should be comprehensive to those who have previously used them.
  %   The tests for KAT languages are a form of boolean algebra.
  %   We add the ``Test'' predicate to allow for the ability to ``test'' a field's value.
  %   This predicate acts as a standard boolean variable in a boolean algebra expression.
  %   \par
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%
    \subsection{Semantics}\label{sec:spec:sem}
      ...
      \sysname operates in this way, and I have labeled the instruction and result streams accordingly.
      ...
      \subsubsection{Semantics of Predicates}\label{sec:spec:sem:pred}
        In this section, we will study just the Semantics of the predicates.
        Please refer to Figure~\ref{fig:spec:sem:pred} for the mathematical definitions.
        Note that the \shdtt{$\defeq$} symbol means \textit{``is defined to be equal to''}.
        So, \shdtt{$\interp{abc}\defeq jkl$} can be read as \textit{``The interpretation of abc is definitionally equivalent to jkl''}.
        Also, the term \textit{ins} in this section refers to when either stream enters the monitor.
        ...
        \paragraph{Identity:}
          \shdtt{$\interp{ 1 }(\mathit{is}, \mathit{rs})$} \par
        ...
        \paragraph{Test}
          \shdtt{$\interp{ f=n }(\mathit{is}, \mathit{rs})$} \par
        ...
        \paragraph{Sum}
          \shdtt{$\interp{ a + b }(\mathit{is}, \mathit{rs})$} \par
        ...
        \paragraph{Product}
          \shdtt{$\interp { a \cdot b }(\mathit{is}, \mathit{rs})$} \par
          ...
          Ins which satisfy both a and b are passed through the monitor, while all others are simply destroyed\footnote{It may not be entirely accurate to say these ins are destroyed, but they are indeed not passed through to the outs.}.

        \paragraph{Negation}
          \shdtt{$\interp { \neg a }(\mathit{is}, \mathit{rs})$} \par
