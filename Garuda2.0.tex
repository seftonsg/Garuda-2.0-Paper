...
%%
%% Custom definitions
\def \sysname {\textsc{G2}}
\def \oldname {\textsc{GARUDA}}
\def \defeq   {\triangleq}
...
Many systems tackle partcular issues such as non-interference, cache side-channel attacks, or speculation exploits \cite{ferraiuolo2017secverilogbl, Spec_TT}.
As the attack model is ever-mutating and often undetected, these solutions can only ever solve part of the problem.
Some solutions propose generic monitors based on programmable hardware monitors \cite{pump_oakland2015} or languages for describing arbitrary policies \cite{GARUDA, anderson2014netkat}.
...
\sysname benefits from a new information model and taint tracking, implemented without changes to the language itself.
These additions incur marginal processor overhead, and ring true to the spatial and power
