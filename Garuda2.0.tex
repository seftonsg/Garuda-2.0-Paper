...
%%%%%%%%%%%%%%%%%%%%%%%%
  \section{Compilation to Verilog}\label{sec:comp}
    ...
    \subsection{The Intermediate Language}\label{sec:comp:int}
      ...
      \shdtt{$let\ (x = read(input\_reg))\ in\ $} \shdtt{$(write(output\_reg,x))$}.
      ...
      \shdtt{$forever\ (let\ (x = read(input\_reg))\ in\ $} \shdtt{$(write(output\_reg,x)))$}.
    ...
    \subsection{Compiling \sysname to the Intermediate}\label{sec:comp:comp}
      ...
      \subsubsection{Compiling Predicates}\label{sec:comp:comp:pred}
        ...
        \textbf{TODO: Check Paragraphs}

        We begin by defining the compilation of Falsity in Figure~\ref{fig:comp:comp:pred}.

        \paragraph{Falsity:}
          $\mathbf{C}[0]$ \par\noindent
          %\shdtt{$\mathbf{C}[0]$} \par
          Falsity does not pass any of its inputs to the outputs.
          As \sysname must process its current element before it may move on to the next, Falsity is compiled to output bogus values.
        ...
        \paragraph{Negation:}
          $\mathbf{C}[\neg a]$ \par\noindent
          Predicate Negation just inverts the behavior of the input.
          Negation can be reduced to a logical \textit{NOT} gate.
      ...
      \subsubsection{Compiling Policies}\label{sec:comp:comp:pol}
        ...
        \textbf{TODO: Find a better way to reference ~\ref{fig:comp:comp:pol}}\par
        \textbf{TODO: Also... capitalization?}
        ...
        \paragraph{Injection to State Register:}
          ...
          State Register injections temporarily override the state register for a cycle while triggering a stall in the previous stages.
          Think of it as splitting the instruction stream and forcing in a new instruction before continuing.

        \paragraph{Injection to Memory Unit:}
          $\mathbf{C}[inj_{Mi}V]$ \par\noindent
          Injection to the Memory Unit is exactly the same as to a state register.
          These two can be used strategically to fool a trojan within the state register.
